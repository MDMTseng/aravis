...
main (int argc, char **argv)
{
	...
	if (device != NULL) {
		...
		v_string = arv_gc_string_get_value (ARV_GC_STRING (node), NULL);
		g_print ("device id     = %s\n", v_string);
		node = arv_gc_get_node (genicam, "DeviceSerialNumber");
		v_string = arv_gc_string_get_value (ARV_GC_STRING (node), NULL);
		g_print ("device sno     = %s\n", v_string);

		node = arv_gc_get_node (genicam, "DeviceUserID");
		v_string = arv_gc_string_get_value (ARV_GC_STRING (node), NULL);
		g_print ("device usrID     = %s\n", v_string);
		...
		arv_camera_set_pixel_format (device, ARV_PIXEL_FORMAT_RGB_8_PACKED, NULL);
		...
		do {
			...
			do  {
				...
				if (buffer != NULL) {
					...
					ArvBufferPayloadType type=	arv_buffer_get_payload_type	(buffer);
					// if(type!=ARV_BUFFER_PAYLOAD_TYPE_IMAGE)
					// {
					// 	continue;
					// }
					gint w=			arv_buffer_get_image_width		(buffer);
					gint h=			arv_buffer_get_image_height		(buffer);

					ArvPixelFormat format=arv_buffer_get_image_pixel_format (buffer);
					size_t dataSize=-1;
					const void *data=		arv_buffer_get_data		(buffer,&dataSize);
					printf ("buffer type:%d  w:%d H:%d  size:%d format:%x\n",type,w,h,dataSize,format);
					

					char nameTmp[200];
					sprintf(nameTmp,"output(%d).bmp",buffer_count);

					// acvSaveBitmapFile(nameTmp, (uint8_t*)data, w/3, h);
					if(buffer_count>3)
						exit(1);
